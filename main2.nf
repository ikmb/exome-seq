...
include { merge_vcf ; vcf_stats ; vcf_index } from "./modules/vcf/main.nf" params(params)
...
workflow {
	...
	main:
		...
		vcf_only = VARIANT_CALLING.out.vcf_only
		...
		if (params.joint_calling) {
			merge_gvcfs(gvcf.collect())
			vcf_index(merge_gvcfs.out)
			merged_vcf = vcf_index.out
		} else {
			merge_vcf(vcf_only.collect())
			vcf_index(merge_vcf.out)
			merged_vcf = vcf_index.out
		}
		...
		// Effect prediction
		vep(vcf.mix(merged_vcf,manta_vcf))
